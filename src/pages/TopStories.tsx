...
export default function TopStories({ navigation }) {
	...
	const { getTopStories } = useHackerNews();
	const wait = useWait();

	const fetch = async (limit: number) => {
		let items = await getTopStories(limit);
	...
	return (
		<View className="flex-1">
			<Header title="Top Stories" navigation={navigation} />
			<ScrollView
				...
				{!loading && stories.length > 0 && (
					<View>
						...
						<View className="flex flex-row justify-center py-10">
							<Pressable
								onPress={() => loadMore()}
								className="border border-gray-500 px-4 py-2 rounded-lg hover:bg-gray-100 active:bg-gray-100"
							>
